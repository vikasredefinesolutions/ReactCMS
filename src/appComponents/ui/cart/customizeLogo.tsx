...
interface _customizedIdProps {
  customize_id: number
}

const CustomizeLogo: NextPage<_customizedIdProps> = (props: _customizedIdProps) => {
  const { customize_id } = props
  ...
  useEffect(() => {
    if (customize_id) {
      getLogoPositionList(customize_id).then((response) => {
