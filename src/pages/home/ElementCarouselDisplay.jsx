...
const ElementCarouselDisplay = ({bannerArr}) => {
    ...
    return ( 
        ...
        {(Object.keys(bannerArr).length > 0 && bannerArr.images != null) && 
            <Carousel renderArrowPrev={(clickHandler, hasPrev, labelPrev) =>
                hasPrev && (
                    <div className="absolute top-1/2 -translate-y-1/2 left-4 z-10 flex items-center">
                            <button onClick={clickHandler} className="bg-light-gray bg-opacity-90 flex justify-center items-center w-10 h-10 rounded-md shadow-md focus:outline-none">
                                <svg viewBox="0 0 20 20" fill="currentColor" className="chevron-left w-10 h-10">
            ...
            renderArrowNext={(clickHandler, hasNext, labelNext) =>
                hasNext && (
                    <div className="absolute top-1/2 -translate-y-1/2 right-4 z-10 flex items-center">
                            <button onClick={clickHandler} className="bg-light-gray bg-opacity-90 flex justify-center items-center w-10 h-10 rounded-md shadow-md focus:outline-none">
                                <svg viewBox="0 0 20 20" fill="currentColor" className="chevron-right w-10 h-10">
            ...
            showStatus={showStatus} stopOnHover={stopOnHover} infiniteLoop={infiniteLoop} autoPlay={false} showArrows={showArrow} showIndicators={showIndicators} showThumbs={showThumb} >
                {bannerArr.images.map((image) => {
                        return (<div className="relative"><div className="overflow-hidden">
                            {image.image_or_video == "Image" ?
                                ...
                                :
                                    <iframe class="p-0 w-full aspect-[7/3]" src={`https://player.vimeo.com/video/${image.video_url}?autoplay=1&amp;loop=1&amp;background=1&amp;muted=1`} allow="autoplay; encrypted-media" frameborder="0"></iframe>
                                }
                                </>
                            }
