import { fetchCategoryByCategoryId, fetchCategoryByproductId } from '@services/product.service';
...
const BreadCrumb: React.FC = () => {
  ...
  const getBreadCrubs = async () => {
    ...
    if (['product', 'category'].includes(pageType.type)) {
      const categories = await (pageType.type === 'category' ? fetchCategoryByCategoryId : fetchCategoryByproductId)(
        ~~pageType.id,
        storeId || 0,
      );
