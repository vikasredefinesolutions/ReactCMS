...
export const multipleImages = (dataArr, selectedObj) => {
  ...
  if (dataArr.length > 0) {
    dataArr.forEach(function (item) {
      strHTML += '<div className="w-full lg:w-1/3 p-4">';
      // strHTML += '<div className="flex justify-center pb-5">';
      // strHTML += '<div className="btn-primary rounded-full w-10 h-10 flex justify-center items-center text-base text-white font-semibold">'+item.index+'</div>';
      // strHTML += '</div>';
      ...
      if (item.Image !== undefined) {
        ...
        strHTML += '<a title="' + item.Image_link + '">';
        strHTML += '<img className="w-full" src="' + item.Image + '"/>';
        strHTML += '</a>';
        strHTML += '</div>';
      }
      if (
        ...
        strHTML += '<div className="text-center bg-white w-full">';
        strHTML += '<div className="text-base p-4">' + item.Headline + '</div>';
        strHTML += '</div>';
      }
      if (item.Button_display != undefined && item.Button_display == 'Yes') {
        strHTML += '<div className="mt-5 mb-5">';
        strHTML +=
          '<a target="" href="' +
          item.Button_link +
          '" className="' +
      ...
      // <div className="mb-5">
      //         <a title="" target="" href="javascript:void(0);" inpname="first_btn" inplnname="first_btn_link" inpclname="first_btn_cls" className="px-6 py-3 text-green-700 font-semibold uppercase bg-neutral-900 hover:bg-gray-600 hrefurl isinput changebtn" data-nofollow="N" contenteditable="true">
      //           lorem impluse
      //         </a>
      //       </div>
      strHTML += '</div>';
